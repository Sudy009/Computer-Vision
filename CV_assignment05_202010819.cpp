...
int main()
{
	...
	*/
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if ((Cb[i * W + j] > 75 && Cb[i * W + j] < 140) && (Cr[i * W + j] > 140 && Cr[i * W + j] < 185)) {
				Output[i * W * 3 + j * 3] = Image[i * W * 3 + j * 3];
				Output[i * W * 3 + j * 3 + 1] = Image[i * W * 3 + j * 3 + 1];
				Output[i * W * 3 + j * 3 + 2] = Image[i * W * 3 + j * 3 + 2];
			}
	...
	for (int i = 1; i < H; i++) {
		for (int j = 1; j < W; j++) {
			if ((Output[i * W * 3 + j * 3] != 0) && (Output[i * W * 3 + j * 3 + 1] != 0) && (Output[i * W * 3 + j * 3 + 2] != 0)){ //전경화소라면
				if (i <= RD_Y) 
					RD_Y = i;
				if (i >= LU_Y)
					LU_Y = i;
	...
	for (int j = LU_X; j < RD_X; j++) {
		...
		Output[RD_Y * W * 3 + j * 3] = Output[RD_Y * W * 3 + j * 3 + 1] = 0;
	}
	for (int j = LU_X; j < RD_X; j++) {
		Output[LU_Y * W * 3 + j * 3 + 2] = 255;
		Output[LU_Y * W * 3 + j * 3] = Output[LU_Y * W * 3 + j * 3 + 1] = 0;
	}
	for (int j = RD_Y; j <= LU_Y; j++) {
		Output[j * W * 3 + RD_X * 3 + 2] = 255;
		Output[j * W * 3 + RD_X * 3] = Output[j * W * 3 + RD_X * 3 + 1] = 0;
	}
	for (int j = RD_Y; j <= LU_Y; j++) {
		Output[j * W * 3 + LU_X * 3 + 2] = 255;
		Output[j * W * 3 + LU_X * 3] = Output[j * W * 3 + LU_X * 3 + 1] = 0;
	}
