...
void zhangSuen(unsigned char* image, unsigned char* output, int rows, int cols)//세선화(Thining)1픽셀인 영상 경로보여줌 
{
	...
	for (int i = 1; i < H - 1; i++) {
		for (int j = 1; j < W - 1; j++) {
			if (Image[i * W + j] == 0) { //전경화소일때
				...
				if (count == 2)//m,n이 0인경우 자기자신 전경화소도 포함되서 2부터
				{
					Output[i * W + j] = 255;
					Output[(i - 1) * W + j] = 128;
					Output[(i + 1) * W + j] = 128;
...
int main()
{
	...
	*/
	...
	SaveBMPFile(hf, hInfo, hRGB, Output, hInfo.biWidth, hInfo.biHeight, "output12.bmp");
